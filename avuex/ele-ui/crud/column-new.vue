<template>
  <!-- 多级表头列 -->
  <dynamic-column v-if="column.children && column.children.length" :columnOption="column" :key="column.prop">
    <template v-for="(item,idx) in crud.propOption" slot-scope="scope" :slot="item.prop">
      <slot :row="scope.row"
            :dic="scope.dic"
            :size="scope.size"
            :label="scope.label"
  ...
  <el-table-column
      v-else-if="column.hide !== true && !(column.children && column.children.length)"
      ...
      :sortable="vaildData(column.sortable,true)"
...
export default {
  ...
  methods: {
    ...
    // 使用xss过滤字符串
    filterValueByXss(str) {
      let nstr = str
      // 传入是否字符串
      const isString = typeof str === 'string'
      // xss对象存在
      if (window._xss && isString) {
        nstr = window._xss(str, {
          // 白名单标签及属性
          whiteList: {
    ...
    */
    handleDetail(row, column) {
      ...
      if (column.viewProp) {
        // return this.$xss(this.getValue(row[column.viewProp]))
        // return this.getValue(row[column.viewProp])
        return this.filterValueByXss(this.getValue(row[column.viewProp]))
      }
      // 自定义的格式化函数
      if (typeof column.formatter === 'function') {
        // return this.getValue(column.formatter(row, this.crud.DIC[column.prop]))
        // formatter涉及到业务自定义标签渲染,暂时不处理xss攻击（需要其他各个域配合）
        // return this.$xss(this.getValue(column.formatter(row, this.crud.DIC[column.prop])))
        return this.filterValueByXss(this.getValue(column.formatter(row, this.crud.DIC[column.prop])))
      }
      // 字典转文字
      if (column.type === 'select' && this.crud.DIC[column.prop] && typeof row[column.prop] === 'number') {
        ...
        const ndata = data ? data[column.props ? column.props.label : 'label'] : '--'
        // return this.$xss(ndata)
        return this.filterValueByXss(ndata)
        // return ndata
      }
      // 直接返回值
      return this.filterValueByXss(this.getValue(row[column.prop]))
    },
    ...
    handleFiltersMethod(value, row, column) {
      const columnOption = [...this.crud.tableColumn]
      const columnNew = columnOption.filter(ele => ele.prop === column.property)[0]
