/**
 * @title 异常类型(经营指标)-树形下拉
 * @author zhc(依据Alinc修改)
 * @date 2020-2-17 20:25:53
 */
<template>
  <div
    ...
    <div class="select-content" :class="(!isSelect && isShow) ? 'is-show' : ''" v-show="isShow">
      ...
      >
        <li
          :class="{active: item.name === filterText,isEnable: item.isEnable === 2}"
          v-for="item in list"
          :key="item.id"
          @click="handleNodeClickForList(item, item.isEnable === 2)"
        >{{ item.abnormalName }}</li>
...
export default create({
  ...
  props: {
    ...
    treeUrl: {
      type: String,
      default: GATEWAY.BUS + '/network/getFirstLevelNetworks'
    },
    // 条件查询接口地址
    remoteUrl: {
      type: String,
      default: GATEWAY.BUS + '/network/select'
    },
    // 条件查询接口地址
    remoteAllUrl: {
      type: String,
      default: GATEWAY.BUS + '/timeEffectiveType/page'
    },
    // 默认样式
    width: {
      type: String,
  ...
  methods: {
    ...
    // 列表节点被点击事件监听
    handleNodeClickForList(row, state) {
      if (state) {
        return;
      }
      // 选中的是同一个节点的话，不执行后续操作
